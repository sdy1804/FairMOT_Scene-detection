...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __init__(self,
        ...
        self.num_classes = 1
        # self.class_map = {"Swimming" : 0, "Turning" : 1, "Diving" : 2}
    ...
    def __getitem__(self, files_index):
        ...
        img_path = self.img_files[ds][files_index - start_index] # path 가져옴
        ...
        for k in range(num_objs):  # 图片中实际的目标个数
            ...
            # cls_id = torch.tensor([cls_id]) # torch tensor 형태로 변경
            # cls_id = F.one_hot(cls_id, num_classes=3)
            # print(label[0])
            # cls_id = self.class_map[label[0]] # label 첫번째 줄의 class만 읽음
            # if label[0] == "Swimming":
            #     cls_id = 0
            # elif label[0] == "Turning":
            #     cls_id = 1
            # else:
            #     cls_id = 2
            # cls_id = np.array([cls_id]) # torch tensor 형태로 변경
            # cls_id = torch.tensor([cls_id]) # torch tensor 형태로 변경
            ...
            if h > 0 and w > 0:
                ...
                # a = draw_gaussian(hm[0], ct_int, radius)  # hm
        ...
        cls_id = int(label[0])  # object's class label(index start from 0)
        cls_id = torch.tensor([label[0]]) # torch tensor 형태로 변경
        # print(label[0])
        cls_id = cls_id.to(torch.int64)
        # print("cls_id:",cls_id)
        cls_id = F.one_hot(cls_id, num_classes=5)
        # print("one-hot cls_id:",cls_id)
        ret = {'input': imgs,
               'hm': hm,
               'reg_mask': reg_mask,
               'ind': ind,
